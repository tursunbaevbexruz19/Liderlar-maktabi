...
/**
 * TextLoop — from React Bits (reactbits.dev), ported to TypeScript.
 *
 * Used in exactly ONE place on this site: the LEARN · GROW · LEAD band. The
 * school's tagline travelling along a curve is the one moment where a piece of
 * pure motion earns its keep, because the words being animated are the brand
 * itself rather than decoration.
 *
 * This is the only component on the site that pulls in GSAP (~23 KB gz), and
 * it is dynamically imported so the cost lands after the page is interactive,
 * never in the LCP path.
 *
 * Reduced motion is respected: the ribbon renders, the text simply doesn't
 * travel.
 */
...
export type TextLoopProps = {
  ...
  className?: string;
  style?: CSSProperties;
  /** Accessible label; the SVG is exposed as an image with this name. */
  label?: string;
};
  ...
  return (
    ...
    >
      <svg
        ...
        // "slice", not the stock "meet" — see TextLoop.css for why.
